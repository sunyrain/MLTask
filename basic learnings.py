...
for i in range(146):
    ...
    if T_Data[:, 1].shape[0] >= 181:  # Determine if the collected data points exceed 180
        ...
        t = t+1
for i in range(213):
    T_Data = pd.read_csv('./train/PressureGain_constant/PressureGain_constant_'+str(i)+'.csv', header=None)  # Load CSV file
    T_Data = np.array(T_Data)  # Convert to numpy format
    if T_Data[:, 1].shape[0] >= 181:  # Determine if the collected data points exceed 180
        Data_E1 = T_Data[1:181, 1:]  # Intercept the first 180 values and discard the header
        Data_E1 = Normal(Data_E1)  # Standardize the signal
        ...
        t = t+1
for i in range(199):
    T_Data = pd.read_csv('./train/PropellerDamage_bad/PropellerDamage_bad_'+str(i)+'.csv', header=None)  # Load CSV file
    T_Data = np.array(T_Data)  # Convert to numpy format
    if T_Data[:, 1].shape[0] >= 181:  # Determine if the collected data points exceed 180
        Data_E1 = T_Data[1:181, 1:]  # Intercept the first 180 values and discard the header
        Data_E1 = Normal(Data_E1)  # Standardize the signal
        ...
        t = t+1
for i in range(208):
    T_Data = pd.read_csv('./train/PropellerDamage_slight/PropellerDamage_slight_'+str(i)+'.csv', header=None)  # Load CSV file
    T_Data = np.array(T_Data)  # Convert to numpy format
    if T_Data[:, 1].shape[0] >= 181:  # Determine if the collected data points exceed 180
        Data_E1 = T_Data[1:181, 1:]  # Intercept the first 180 values and discard the header
        Data_E1 = Normal(Data_E1)  # Standardize the signal
...
for i in range(36):
    ...
    if T_Data[:, 1].shape[0] >= 181:  # Determine if the collected data points exceed 180
        ...
        t = t+1
for i in range(53):
    T_Data = pd.read_csv('./test/PressureGain_constant/PressureGain_constant_'+str(i)+'.csv', header=None)  # Load CSV file
    T_Data = np.array(T_Data)  # Convert to numpy format
    if T_Data[:, 1].shape[0] >= 181:  # Determine if the collected data points exceed 180
        Data_E1 = T_Data[1:181, 1:]  # Intercept the first 180 values and discard the header
        Data_E1 = Normal(Data_E1)  # Standardize the signal
        ...
        t = t+1
for i in range(50):
    T_Data = pd.read_csv('./test/PropellerDamage_bad/PropellerDamage_bad_'+str(i)+'.csv', header=None)  # Load CSV file
    T_Data = np.array(T_Data)  # Convert to numpy format
    if T_Data[:, 1].shape[0] >= 181:  # Determine if the collected data points exceed 180
        Data_E1 = T_Data[1:181, 1:]  # Intercept the first 180 values and discard the header
        Data_E1 = Normal(Data_E1)  # Standardize the signal
        ...
        t = t+1
for i in range(52):
    T_Data = pd.read_csv('./test/PropellerDamage_slight/PropellerDamage_slight_'+str(i)+'.csv', header=None)  # Load CSV file
    T_Data = np.array(T_Data)  # Convert to numpy format
    if T_Data[:, 1].shape[0] >= 181:  # Determine if the collected data points exceed 180
        Data_E1 = T_Data[1:181, 1:]  # Intercept the first 180 values and discard the header
        Data_E1 = Normal(Data_E1)  # Standardize the signal
